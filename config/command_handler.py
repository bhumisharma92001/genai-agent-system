...
class CommandHandler:
    ...
    def handle_ingest(self, query: str, user_id: str, session_id: str) -> None:
        target_path = query[7:].strip()
        if not target_path:
            print("\n[System]: Usage: /ingest <path>")
            return
        try:
            self.pipeline.index_document(file_path=target_path, user_id=user_id, session_id=session_id)
    ...
    def handle_startup_ingestion(self, user_id: str, session_id: str) -> None:
        ...
        if not existing_docs:
            ...
            if file_path:
                try:
                    self.pipeline.index_document(file_path=file_path, user_id=user_id, session_id=session_id)
